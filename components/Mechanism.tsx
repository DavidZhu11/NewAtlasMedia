...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, step }) => (
  <div className="group relative p-6 sm:p-8 rounded-2xl bg-white/5 border border-white/5 hover:border-brand-purple/50 transition-all hover:bg-white/10 overflow-hidden h-full">
    <div className="absolute top-0 right-0 p-3 sm:p-4 opacity-10 font-black text-5xl sm:text-6xl text-white group-hover:opacity-20 transition-opacity">
      0{step}
    </div>
    <div className="relative z-10 flex flex-col h-full">
      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-brand-purple/10 flex items-center justify-center text-brand-purple mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-300 border border-brand-purple/20">
        {icon}
      </div>
  ...
  return (
    <section id="mechanism" className="py-16 sm:py-24 bg-brand-dark relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 md:grid-cols-3 relative">
          {/* Connector Line */}
